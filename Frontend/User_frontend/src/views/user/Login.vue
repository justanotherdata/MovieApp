    ...
    <div class="container container_style" v-if="!user_login">
        ...
        <form>
            ...
            <div class="mb-3 form-check">
                <input v-model= 'isactive' type="checkbox" class="form-check-input form_style" id="exampleCheck1">
                <label class="form-check-label" for="exampleCheck1">Remember me</label>
            </div>
            <button @click= 'login_' type ='button' class="btn btn-primary col-6">Login</button>
        </form>
    </div>

    <div v-if="user_login" class="container container_style">
...
export default {
  ...
  mounted(){
    this.$store.dispatch('get_curr_page', 'login')
  },
  methods: {
    gohome(){
    ...
    login_(){
      ...
      fetch("http://127.0.0.1:5000/login_user", requestOptions)
        ...
        .then(result => {
          ...
          if(this.data['token']){
            ...
            this.$store.dispatch('get_user_role', 'User')
            this.$router.push({path:'/'})
